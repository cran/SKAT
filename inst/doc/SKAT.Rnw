...
In the following example, we conducted sample size calculation using the haplotypes in SKAT.haplotypes with 
...
If different r.corr ($\rho$) values to be used to compute the power, 
